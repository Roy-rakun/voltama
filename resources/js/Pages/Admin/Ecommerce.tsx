...
import { Plus, Trash2, ExternalLink, ShoppingBag, Upload, Store } from 'lucide-react';
...
export default function Ecommerce({ stores, success }: EcommerceProps) {
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        post(route('admin.ecommerce.store'), {
            forceFormData: true,
            onSuccess: () => {
                reset();
    ...
    const handleDelete = (index: number, name: string) => {
        if (confirm(`Hapus toko "${name}"?`)) {
            router.delete(route('admin.ecommerce.destroy', index));
        }
    };
    ...
    return (
        <AuthenticatedLayout
            header={
                <div>
                    <p className="text-xs text-gray-400">Pages / <span className="text-gray-600">E-Commerce</span></p>
                    <h2 className="text-lg font-bold text-gray-800 leading-tight">Kelola Toko E-Commerce</h2>
                </div>
            }
        >
            <Head title="Kelola E-Commerce — Voltama Admin" />
                ...
                <div className="lg:col-span-1">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <div className="flex items-center gap-3 mb-5">
                            ...
                            <div>
                                <h3 className="font-bold text-gray-800 text-sm">Tambah Toko Baru</h3>
                                <p className="text-xs text-gray-400">Tokopedia, Shopee, Lazada, dll.</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            {/* Nama Toko */}
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-1.5">Nama Platform</label>
                                <input
                                    type="text"
                                    value={data.name}
                            ...
                            {/* Link Toko */}
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-1.5">Link URL Toko</label>
                                <input
                                    type="url"
                                    value={data.link}
                                    onChange={(e) => setData('link', e.target.value)}
                                    placeholder="https://shopee.co.id/voltama"
                                    className="w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:border-[#ffc400] focus:ring-1 focus:ring-[#ffc400] transition"
                                />
                            ...
                            {/* Upload Logo */}
                            <div>
                                <label className="block text-xs font-bold text-gray-600 mb-1.5">Logo / Ikon Marketplace</label>
                                {logoPreview && (
                            ...
                            >
                                <Plus size={16} />
                                {processing ? 'Menyimpan...' : 'Tambah Toko'}
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="mt-4 bg-blue-50 border border-blue-100 rounded-2xl p-4 text-xs text-blue-700 space-y-1.5">
                        ...
                        <p>Logo akan tampil di <strong>section Promo</strong> (tengah halaman) dan <strong>Footer</strong> website.</p>
                        <p>Gunakan logo dengan background transparan (PNG/SVG) untuk hasil terbaik.</p>
                    </div>
                </div>
                ...
                <div className="lg:col-span-2">
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                        <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
                            <div>
                                <h3 className="font-bold text-gray-800 text-sm">Daftar Toko E-Commerce</h3>
                                <p className="text-xs text-gray-400 mt-0.5">{stores.length} toko terdaftar</p>
                            </div>
                        ...
                        {stores.length === 0 ? (
                            <div className="text-center py-16 text-gray-400">
                                ...
                                <p className="text-sm font-medium">Belum ada toko yang ditambahkan.</p>
                                <p className="text-xs mt-1">Tambah toko pertama menggunakan form di sebelah kiri.</p>
                            </div>
                        ) : (
                                    ...
                                    <div key={index} className="flex items-center gap-4 px-6 py-4 hover:bg-gray-50/60 transition group">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <a
                                                ...
                                                className="text-xs text-gray-400 hover:text-[#ffc400] transition flex items-center gap-1 mt-0.5 truncate max-w-xs"
                                            >
                                        ...
                                        <div className="flex items-center gap-2 shrink-0">
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="p-2 rounded-lg text-gray-400 hover:text-[#ffc400] hover:bg-yellow-50 transition"
                                                title="Buka toko"
                                            >
                                                <ExternalLink size={15} />
                                            </a>
                                            <button
                                                onClick={() => handleDelete(index, store.name)}
                    ...
                    {stores.length > 0 && (
                        <div className="mt-4 bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Preview — Tampil di Website</p>
                            <div className="flex flex-wrap gap-3">
                                {stores.map((store, index) => (
                                    <div
                                        key={index}
                                        className="flex items-center gap-2 bg-white border border-gray-100 shadow-sm px-4 py-2.5 rounded-xl hover:scale-105 transition cursor-default"
                                    >
                                        <span className="text-xs text-gray-400 font-medium">Tersedia di</span>
