...
import { Plus, Pencil, Trash2, Image as ImageIcon, BookOpen, Check } from 'lucide-react';
...
export default function ArticleCategories({ categories, success }: Props) {
    ...
    const handleAdd = (e: FormEvent) => {
        e.preventDefault();
        addForm.post(route('admin.article-categories.store'), {
            forceFormData: true,
            onSuccess: () => {
                addForm.reset();
    ...
    const handleEdit = (e: FormEvent, id: number) => {
        e.preventDefault();
        editForm.post(route('admin.article-categories.update', id), {
            forceFormData: true,
    ...
    const handleDelete = (id: number, name: string) => {
        if (confirm(`Hapus kategori "${name}"? Artikel yang ada tidak akan terhapus.`)) {
            router.delete(route('admin.article-categories.destroy', id));
        }
    };
    ...
    return (
        <AuthenticatedLayout header={
            <div>
                <p className="text-xs text-gray-400">Admin / Artikel / <span className="text-gray-600">Kategori</span></p>
                <h2 className="text-lg font-bold text-gray-800 dark:text-gray-200">Kategori Artikel</h2>
            </div>
        }>
            ...
            <div className="mb-6 bg-[#1a1e2e] rounded-2xl p-5 flex items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-sm font-bold text-white">Kategori Artikel</p>
                        <p className="text-xs text-gray-400">{categories.length} kategori tersedia</p>
                    </div>
                </div>
            ...
            {showAddForm && (
                <div className="mb-6 bg-white dark:bg-gray-800 rounded-2xl border border-[#ffc400] shadow-lg p-6">
                    <h3 className="font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
                        <Plus size={16} className="text-[#ffc400]" /> Tambah Kategori Artikel Baru
                    </h3>
                    <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" value={addForm.data.name} onChange={e => addForm.setData('name', e.target.value)}
                                className={inputClass} placeholder="contoh: Tips Kelistrikan" required />
                        </div>
                        <div>
            ...
            {categories.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-16 text-center">
                    ...
                    <p className="font-bold text-gray-400">Belum ada kategori artikel.</p>
                </div>
            ) : (
                        ...
                        <div key={cat.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden hover:shadow-lg transition-shadow">
                            {editingId === cat.id ? (
                                <form onSubmit={(e) => handleEdit(e, cat.id)} className="p-5 space-y-3">
                                    ...
                                    <div>
                                        {cat.image_path && (
                                            <img src={cat.image_path} alt={cat.name} className="h-14 object-contain mb-2 rounded-lg bg-gray-50 dark:bg-gray-900 p-1" />
                                        )}
                                        <input type="file" accept="image/*"
                                            onChange={e => editForm.setData('image_file', e.target.files ? e.target.files[0] : null)}
                                            className="block w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-3 file:rounded-lg file:border-0 file:text-xs file:bg-yellow-50 file:text-yellow-700 cursor-pointer" />
                                    </div>
                                ...
                                <div>
                                    <div className="h-32 bg-gray-50 dark:bg-gray-900 flex items-center justify-center overflow-hidden border-b border-gray-100 dark:border-gray-700 relative">
                                        {cat.image_path ? (
                                    ...
                                    <div className="p-4">
                                        ...
                                        <p className="text-xs text-gray-400 mt-1">{cat.articles_count} artikel dalam kategori ini</p>
